...
// ---------- 音频 URL 元数据缓存 (W15) ----------
// 只缓存 content-type / content-length，不缓存音频字节（避免版权与内存问题）
const AUDIO_META_CACHE_MAX = 200;
const AUDIO_META_CACHE_TTL = 5 * 60 * 1000; // 5 分钟
const audioMetaCache = new Map();
function getAudioMeta(url) {
  const e = audioMetaCache.get(url);
  if (!e) return null;
  if (Date.now() - e.t > AUDIO_META_CACHE_TTL) {
    audioMetaCache.delete(url);
    return null;
  }
  return e.meta;
}
function setAudioMeta(url, meta) {
  if (audioMetaCache.size >= AUDIO_META_CACHE_MAX) {
    // 简单 LRU：删最早插入的一项
    const firstKey = audioMetaCache.keys().next().value;
    if (firstKey) audioMetaCache.delete(firstKey);
  }
  audioMetaCache.set(url, { meta, t: Date.now() });
}
...
// ---------- Session Management (多用户 Cookie 隔离) ----------
...
function getOrCreateSession(req) {
  ...
  const match = cookieHeader.match(/mineradio_sid=([^;]+)/);
  let sid = match ? match[1] : null;

  if (!sid || !sessions.has(sid)) {
    sid = crypto.randomBytes(16).toString('hex');
    sessions.set(sid, { neteaseCookie: '', qqCookie: '', createdAt: Date.now(), lastAccess: Date.now() });
  } else {
...
function cleanupExpiredSessions() {
  ...
  for (const [sid, data] of sessions) {
    if ((data.lastAccess || data.createdAt) < now - EXPIRE_MS) {
      sessions.delete(sid);
      cleaned++;
    }
  }
  if (cleaned > 0) {
    console.log('[Session] Cleaned up ' + cleaned + ' expired sessions, remaining: ' + sessions.size);
  }
}
...
function getUserCookie() {
  ...
  return ctx ? ctx.neteaseCookie : '';
}
function setUserCookie(c) {
  const cookie = normalizeCookieHeader(c) || rawCookieFallback(c);
  const ctx = sessionStore.getStore();
  if (ctx) ctx.neteaseCookie = cookie;
}
function getQQCookie() {
  const ctx = sessionStore.getStore();
  return ctx ? ctx.qqCookie : '';
}
function setQQCookie(c) {
  const cookie = normalizeCookieHeader(c) || rawCookieFallback(c);
  const ctx = sessionStore.getStore();
  if (ctx) ctx.qqCookie = cookie;
}

function saveCookie(c) { setUserCookie(c); }
function saveQQCookie(c) { setQQCookie(c); }
...
async function handleSearch(keywords, limit) {
  ...
  const result = await cloudsearch({ keywords, limit, cookie: getUserCookie() });
  ...
  if (missing.length) {
    try {
      ...
      const dd = await song_detail({ ids: missing.join(','), cookie: getUserCookie() });
...
async function handleDiscoverHome() {
  ...
  const tasks = [
    personalized({ limit: 8, cookie: getUserCookie(), timestamp: Date.now() }),
    dj_hot({ limit: 6, offset: 0, cookie: getUserCookie(), timestamp: Date.now() }),
    recommend_resource({ cookie: getUserCookie(), timestamp: Date.now() }),
    recommend_songs({ cookie: getUserCookie(), timestamp: Date.now() }),
  ];
...
async function fetchWeatherPlaylistSongs(playlist, limit) {
  ...
  try {
    if (typeof playlist_track_all === 'function') {
      const all = await playlist_track_all({ id, limit: limit || 36, offset: 0, cookie: getUserCookie(), timestamp: Date.now() });
  ...
  if (!rawTracks.length && typeof playlist_detail === 'function') {
    try {
      const detail = await playlist_detail({ id, s: 0, cookie: getUserCookie(), timestamp: Date.now() });
...
async function filterLikelyPlayableWeatherSongs(songs) {
  ...
  for (let i = 0; i < source.length; i += 4) {
    ...
    const settled = await Promise.allSettled(chunk.map(async song => {
      const info = await handleSongUrl(song.id, { loggedIn: !!getUserCookie() }, 'standard');
...
async function qqMusicRequest(payload, opts) {
  ...
  if (opts.cookie && getQQCookie()) headers.Cookie = getQQCookie();
...
async function getQQLoginInfo() {
  ...
  if (!uin || !musicKey) return { provider: 'qq', loggedIn: false, hasCookie: !!getQQCookie() };
  ...
  try {
    ...
    const text = await requestText(u.toString(), {
      headers: { ...QQ_HEADERS, Cookie: getQQCookie() },
    });
...
async function qqGetJSON(targetUrl, params, opts) {
  ...
  if (opts.cookie !== false && getQQCookie()) headers.Cookie = getQQCookie();
...
function audioProxyHeadersFor(audioUrl, range) {
  ...
  } catch (e) { console.warn('[audioProxyHeadersFor]', e && e.message); }
...
function audioContentTypeForUrl(audioUrl, upstreamType) {
  let pathname = '';
  try { pathname = new URL(audioUrl).pathname.toLowerCase(); } catch (e) { console.warn('[audioContentTypeForUrl]', e && e.message); }
...
async function fetchMyPodcastItems(key, info, limit, offset) {
  ...
  if (key === 'collect') {
    const r = await dj_sublist({ limit, offset, cookie: getUserCookie(), timestamp: Date.now() });
  ...
  if (key === 'created') {
    const r = await user_audio({ uid: info.userId, cookie: getUserCookie(), timestamp: Date.now() });
    const raw = firstArrayFrom(r.body, ['data', 'djRadios', 'djradios', 'radios']);
    return { itemType: 'radio', items: raw.map(x => mapPodcastCollectionRadio(x, key)).filter(x => x.id) };
  }
  if (key === 'paid') {
    const r = await dj_paygift({ limit, offset, cookie: getUserCookie(), timestamp: Date.now() });
    const raw = firstArrayFrom(r.body, ['data', 'djRadios', 'djradios', 'radios']);
    return { itemType: 'radio', items: raw.map(x => mapPodcastCollectionRadio(x, key)).filter(x => x.id) };
  }
  if (key === 'liked') {
    let raw = [];
    try {
      const sati = await sati_resource_sub_list({ cookie: getUserCookie(), timestamp: Date.now() });
    ...
    if (!raw.length) {
      try {
        const recent = await record_recent_voice({ limit, cookie: getUserCookie(), timestamp: Date.now() });
...
async function getLoginInfo() {
  if (!getUserCookie()) return { loggedIn: false, vipType: 0, vipLevel: 'none', isVip: false, isSvip: false, vipLabel: '无VIP' };

  // login_status 对二维码 cookie 的资料刷新通常更及时；失败时再降级到 user_account。
  try {
    const st = await login_status({ cookie: getUserCookie(), timestamp: Date.now() });
    const body = st.body || {};
  ...
  try {
    const acc = await user_account({ cookie: getUserCookie(), timestamp: Date.now() });
    const body = acc.body || {};
    const info = normalizeLoginInfo(body.profile, body.account, body);
    if (info.loggedIn) return info;
    if (isNeteaseAuthInvalidPayload(acc)) saveCookie('');
    return { loggedIn: false, hasCookie: !!getUserCookie(), vipType: 0, vipLevel: 'none', isVip: false, isSvip: false, vipLabel: '无VIP' };
  } catch (e) {
...
function corsOriginFor(req) {
  ...
  if (CORS_ALLOWED_ORIGINS.length === 0) return null; // 未配置则不允许跨域
...
const server = http.createServer(async (req, res) => {
  ...
  const cookieHeader = req.headers.cookie || '';
  if (!cookieHeader.includes(`mineradio_sid=${session.sid}`)) {
    res.setHeader('Set-Cookie', sessionCookieHeader(session.sid));
  }

  await sessionStore.run(session.data, async () => {
    ...
    try {
      ...
      req.on('end', function() {
        try {
          ...
          console.error('[ClientError]', err.type || 'error', err.message || '-', err.file || '');
        } catch (e) { console.warn('[log-client-error parse]', e && e.message); }
  ...
  if (pn === '/api/podcast/search') {
    try {
      ...
      const r = await cloudsearch({ keywords: kw, type: 1009, limit, cookie: getUserCookie(), timestamp: Date.now() });
  ...
  if (pn === '/api/podcast/hot') {
    try {
      const limit = Math.max(6, Math.min(30, parseInt(url.searchParams.get('limit') || '18', 10) || 18));
      const offset = Math.max(0, parseInt(url.searchParams.get('offset') || '0', 10) || 0);
      const r = await dj_hot({ limit, offset, cookie: getUserCookie(), timestamp: Date.now() });
      const body = r.body || {};
  ...
  if (pn === '/api/podcast/detail') {
    try {
      ...
      const r = await dj_detail({ rid, cookie: getUserCookie(), timestamp: Date.now() });
      const body = r.body || {};
  ...
  if (pn === '/api/podcast/programs') {
    try {
      ...
      const offset = Math.max(0, parseInt(url.searchParams.get('offset') || '0', 10) || 0);
      const r = await dj_program({ rid, limit, offset, asc: false, cookie: getUserCookie(), timestamp: Date.now() });
      const body = r.body || {};
  ...
  // ---------- 登出 ----------
  if (pn === '/api/logout') {
    try { await logout({ cookie: getUserCookie() }); } catch (e) { console.warn('[logout]', e && e.message); }
    saveCookie('');
  ...
  if (pn === '/api/user/playlists') {
    try {
      ...
      const r = await user_playlist({ uid: info.userId, limit, cookie: getUserCookie(), timestamp: Date.now() });
    ...
    try {
      ...
      try {
        if (typeof song_like_check === 'function') {
          const checked = await song_like_check({ ids: JSON.stringify(ids.map(Number).filter(Boolean)), cookie: getUserCookie(), timestamp: Date.now() });
      ...
      if (!likedIds.length) {
        const r = await likelist({ uid: info.userId, cookie: getUserCookie(), timestamp: Date.now() });
  ...
  if (pn === '/api/song/like') {
    try {
      ...
      const r = await like_song({ id, like: String(nextLike), cookie: getUserCookie(), timestamp: Date.now() });
  ...
  if (pn === '/api/playlist/create') {
    try {
      ...
      const r = await playlist_create({ name, privacy, cookie: getUserCookie(), timestamp: Date.now() });
  ...
  if (pn === '/api/playlist/add-song') {
    try {
      ...
      const primary = await playlist_tracks({ op: 'add', pid, tracks: String(id), cookie: getUserCookie(), timestamp: Date.now() });
      ...
      if (!success && typeof playlist_track_add === 'function') {
        try {
          const fallback = await playlist_track_add({ pid, ids: String(id), cookie: getUserCookie(), timestamp: Date.now() });
    ...
    try {
      ...
      try {
        if (typeof lyric_new === 'function') {
          const nr = await lyric_new({ id, cookie: getUserCookie(), timestamp: Date.now() });
          body = nr.body || {};
          source = 'lyric_new';
        }
      } catch (errNew) {
      ...
      if (!((body.lrc && body.lrc.lyric) || (body.yrc && body.yrc.lyric))) {
        const r = await lyric({ id, cookie: getUserCookie(), timestamp: Date.now() });
        body = r.body || body || {};
        source = 'lyric';
      }
      sendJSON(res, {
  ...
  if (pn === '/api/song/comments') {
    try {
      ...
      const r = await comment_music({ id, limit, offset, cookie: getUserCookie(), timestamp: Date.now() });
      const body = r.body || r || {};
  ...
  if (pn === '/api/artist/detail') {
    try {
      ...
      let detailBody = {};
      try {
        const detail = await artist_detail({ id, cookie: getUserCookie(), timestamp: Date.now() });
      ...
      let rawSongs = [];
      try {
        const list = await artist_songs({ id, order: 'hot', limit, offset: 0, cookie: getUserCookie(), timestamp: Date.now() });
      ...
      if (!rawSongs.length) {
        const top = await artist_top_song({ id, cookie: getUserCookie(), timestamp: Date.now() });
  ...
  if (pn === '/api/playlist/tracks') {
    try {
      ...
      if (typeof playlist_track_all === 'function') {
        try {
          const all = await playlist_track_all({ id, limit: 500, offset: 0, cookie: getUserCookie(), timestamp: Date.now() });
          rawTracks = (all.body && (all.body.songs || all.body.tracks)) || [];
        } catch (err) {
  ...
  if (pn === '/api/cover') {
    try {
      ...
      // SSRF 防护：必须是 http(s) 且域名在白名单内
      if (!coverUrl || !/^https?:\/\//i.test(coverUrl) || !isAllowedProxyTarget(coverUrl)) {
        res.writeHead(400);
  ...
  if (pn === '/api/audio') {
    try {
      const audioUrl = url.searchParams.get('url');
      // SSRF 防护：必须是 http(s) 且域名在白名单内
      if (!audioUrl || !/^https?:\/\//i.test(audioUrl) || !isAllowedProxyTarget(audioUrl)) {
        res.writeHead(400);
  ...
  // ---------- QQ 音乐 QR 扫码登录 ----------
  if (pn === '/api/qq/login/qr') {
    try {
      const result = await qqMusicCreateQRLogin();
      sendJSON(res, result);
    } catch (err) {
  ...
  if (pn === '/api/qq/login/qr/status') {
    try {
      const key = url.searchParams.get('key') || '';
      const result = await qqMusicCheckQRLogin(key);
      sendJSON(res, result);
    } catch (err) {
...
// Cookie Jar：简单实现，跨域名维护 Set-Cookie
class SimpleCookieJar {
  constructor() { this.cookies = new Map(); }
  set(url, setCookieHeaders) {
  ...
  get(domain) {
    const entries = [];
    for (const [k, v] of this.cookies) entries.push(k + '=' + v);
    return entries.join('; ');
  }
  getCookie(name) {
...
async function qqMusicCreateQRLogin() {
  ...
  // Step 1: xlogin 拿 pt_login_sig
  const xloginUrl = 'https://xui.ptlogin2.qq.com/cgi-bin/xlogin?' + new URLSearchParams({
    appid: '716027609', daid: '383', style: '33', login_text: '授权并登录',
  ...
  const r1 = await fetch(xloginUrl, {
    headers: { 'User-Agent': UA },
    redirect: 'manual',
  });
  jar.set('ptlogin2.qq.com', r1.headers.getSetCookie ? r1.headers.getSetCookie() : r1.headers.get('set-cookie'));
  const ptLoginSig = jar.getCookie('pt_login_sig');

  // Step 2: ptqrshow 拿二维码图片 + qrsig
  const qrUrl = 'https://ssl.ptlogin2.qq.com/ptqrshow?' + new URLSearchParams({
    appid: '716027609', e: '2', l: 'M', s: '3', d: '72', v: '4',
    t: Math.random().toString(), daid: '383', pt_3rd_aid: '100497308',
  });
  const r2 = await fetch(qrUrl, {
    headers: { 'User-Agent': UA },
    redirect: 'manual',
  });
  jar.set('ptlogin2.qq.com', r2.headers.getSetCookie ? r2.headers.getSetCookie() : r2.headers.get('set-cookie'));
  const qrsig = jar.getCookie('qrsig');
  const ptqrtoken = qqHash33(qrsig);
  const qrImageBase64 = Buffer.from(await r2.arrayBuffer()).toString('base64');

  QR_LOGIN_JOBS.set(key, {
    jar, ptLoginSig, qrsig, ptqrtoken,
    status: 'waiting',
    createdAt: Date.now(),
  });
  ...
  for (const [k, v] of QR_LOGIN_JOBS) {
    if (Date.now() - v.createdAt > QR_LOGIN_TIMEOUT) QR_LOGIN_JOBS.delete(k);
  }

  return {
    qrImage: 'data:image/png;base64,' + qrImageBase64,
    key,
    expiresAt: Date.now() + QR_LOGIN_TIMEOUT,
  };
}

async function qqMusicCheckQRLogin(key) {
  const job = QR_LOGIN_JOBS.get(key);
  if (!job) return { status: 'expired', error: 'QR job not found or expired' };
  if (Date.now() - job.createdAt > QR_LOGIN_TIMEOUT) {
    QR_LOGIN_JOBS.delete(key);
    return { status: 'expired' };
  }
  if (job.status === 'confirmed') return { status: 'confirmed', loggedIn: true };
  if (job.status === 'error') return { status: 'error', error: job.error };

  // Step 3: ptqrlogin 轮询扫码状态
  const loginUrl = 'https://ssl.ptlogin2.qq.com/ptqrlogin?' + new URLSearchParams({
    u1: 'https://graph.qq.com/oauth2.0/login_jump',
    ptqrtoken: String(job.ptqrtoken),
    ptredirect: '0', h: '1', t: '1', g: '1',
    from_ui: '1', ptlang: '2052',
    action: '0-0-' + Date.now(),
    js_ver: '20102616', js_type: '1',
    login_sig: job.ptLoginSig,
    pt_uistyle: '40',
    aid: '716027609', daid: '383',
    pt_3rd_aid: '100497308',
    has_onekey: '1',
  });
  const r3 = await fetch(loginUrl, {
    headers: {
      ...
      'Cookie': job.jar.get('ptlogin2.qq.com'),
    },
    redirect: 'manual',
  });
  job.jar.set('ptlogin2.qq.com', r3.headers.getSetCookie ? r3.headers.getSetCookie() : r3.headers.get('set-cookie'));
  const text = await r3.text();

  // 解析状态
  if (text.includes('二维码已经失效') || text.includes('二维码已过期')) {
    QR_LOGIN_JOBS.delete(key);
    return { status: 'expired' };
  }
  if (text.includes('二维码认证中') || text.startsWith("ptuiCB('67'")) {
    return { status: 'scanned', message: '已扫码，请在手机上确认' };
  }
  if (text.startsWith("ptuiCB('66'")) {
    return { status: 'waiting', message: '等待扫码' };
  }

  // 登录成功：ptuiCB('0','0','https://ssl.ptlogin2.../check_sig?...',...)
  if (text.startsWith("ptuiCB('0'")) {
    const m = text.match(/'(https:[^']+)'/);
    if (!m) return { status: 'error', error: 'CHECK_SIG_URL_NOT_FOUND' };
    const checkSigUrl = m[1];

    // Step 4: 跟随 check_sig，禁止重定向，拿 code
    const r4 = await fetch(checkSigUrl, {
      headers: { 'User-Agent': UA, 'Cookie': job.jar.get('ptlogin2.qq.com') },
      redirect: 'manual',
    });
    job.jar.set('graph.qq.com', r4.headers.getSetCookie ? r4.headers.getSetCookie() : r4.headers.get('set-cookie'));
    const loc = r4.headers.get('location') || '';
    let code = '';
    try { code = new URL(loc, 'https://graph.qq.com/').searchParams.get('code') || ''; } catch (e) { /* ignore */ }

    if (!code) return { status: 'error', error: 'OAUTH_CODE_NOT_FOUND' };

    // Step 5: musicu.fcg 用 code 换 musickey
    const body = {
    ...
    const r5 = await fetch('https://u.y.qq.com/cgi-bin/musicu.fcg', {
      method: 'POST',
      headers: {
        'User-Agent': UA,
    ...
    const j = await r5.json();
    const data = j && j.token && j.token.data;
    if (!data || !data.musickey) {
      return { status: 'error', error: 'MUSICKEY_NOT_RECEIVED', raw: j };
    }

    // 构建 cookie 字符串并存入 session
    const uin = String(data.musicid || data.uin || '');
    const musicKey = data.musickey;
    const cookieStr = 'uin=' + uin + '; qqmusic_key=' + musicKey + '; qm_keyst=' + musicKey + ';';
    ...
    // 获取用户信息
    let info;
    try { info = await getQQLoginInfo(); } catch (e) { info = { loggedIn: true, userId: uin }; }
...
// ====================================================================
// N9: WebSocket 连接管理（在线人数实时推送）
// ====================================================================
const wsClients = new Set();
function wsBroadcastOnlineCount() {
  const msg = JSON.stringify({ type: 'online', count: wsClients.size });
...
server.on('upgrade', (req, socket, head) => {
  ...
  ws.pingTimer = setInterval(() => {
    ...
    try {
      ...
      header[1] = 0;
      socket.write(header);
    } catch (e) { clearInterval(ws.pingTimer); }
  }, 30000);
});
